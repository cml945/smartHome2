...
go2rtc 原生运行在 macOS 宿主机上，负责接入小米摄像头并输出 RTSP 流给 Frigate。它支持小米摄像头的 `xiaomi://` 协议，无需刷固件或提取 RTSP Token。
...
```yaml
# go2rtc/config.yml（由 go2rtc/config.example.yml 复制后填写）
streams:
  living_room_cam:
...
Frigate 是目前 HA 生态中最成熟的 NVR 方案。本项目当前使用 Frigate 0.15.0，默认配置为 CPU detector；Apple Silicon detector 相关文件保留为升级到支持版本后的优化选项。

**检测架构：** Frigate 本体运行在 Docker 中，默认使用 CPU detector 做 person 检测。若升级到支持 Apple Silicon detector 的 Frigate 版本，可让检测模型通过 macOS 原生进程运行在宿主机上，再由 Frigate 通过 ZeroMQ 访问。

**性能表现：** 当前模板默认每路 1FPS，适合先稳定跑通。若后续启用 Apple Silicon detector，可再提高检测帧率或降低 CPU 占用。
...
# Frigate 配置示例
mqtt:
  host: mosquitto  # Docker Compose 中的 Mosquitto 服务名
  port: 1883
...
cameras:
  ...
  bedroom:
    ffmpeg:
      inputs:
        - path: rtsp://host.docker.internal:8554/bedroom_cam
          roles: ["detect"]
    detect:
      width: 1280
      height: 720
...
**Frigate 集成：** 通过 HACS 安装 Frigate Integration，它会自动在 HA 中创建 camera、zone、object 对应的 occupancy binary_sensor。为避免实体名冲突，建议 Frigate 的 zone 名称全局唯一；当前模板和自动化按 `binary_sensor.<zone名>_person_occupancy` 口径编写，例如 `binary_sensor.desk_area_person_occupancy`。

**小米设备集成（推荐 ha_xiaomi_home）：** 推荐优先使用小米官方 Home Assistant 集成 `ha_xiaomi_home` 来控制灯具，灯具会被暴露为标准的 `light.*` 或 `switch.*` 实体。注意：虽然小米集成也可能创建摄像头实体，但视频流统一由 go2rtc/Frigate 处理。
...
automation:
  - alias: "客厅沙发区灯光联动"
    trigger:
      - platform: state
        entity_id: binary_sensor.sofa_area_person_occupancy
        to: "on"
    action:
  ...
  - alias: "卧室书桌区灯光联动"
    trigger:
      - platform: state
        entity_id: binary_sensor.desk_area_person_occupancy
        to: "on"
    condition:
...
Frigate    ──── MQTT             ────▶ Docker Mosquitto Broker (:1883)
HA         ──── MQTT             ────▶ Docker Mosquitto Broker (:1883)
...
| Frigate 推理 | CPU detector | 视摄像头数而定 | 当前 Frigate 0.15.0 默认配置 |
...
2. 准备 go2rtc 本地配置并用 launchd 运行 go2rtc
3. 通过 Docker Compose 部署 Frigate + Mosquitto
...
| 组件 | 选型 | 理由 |
|------|------|------|
| 视频流接入 | go2rtc (macOS 原生) | 原生支持小米 P2P 协议，免刷固件，避开容器 P2P UDP 路由问题 |
| NVR + 人体检测 | Frigate 0.15.0 | HA 生态成熟，原生支持 Zone |
| AI 推理 | CPU detector（默认） / Apple Silicon detector（升级预留） | 默认优先稳定跑通，升级后再利用 Neural Engine |
